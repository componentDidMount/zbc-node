...
const {topicClientErrors} = require('./__internal/errors');
...
class TopicClient {
  ...
  createTopic (topicName, partitionCount, replicationFactor) {
    if (!typeMatchers.string(topicName)) {
      console.error(`${topicName} ${topicClientErrors.createTopic_TopicNameString}`)
    }
    if (isUndefinedOrNull(partitionCount)) {
      ...
      console.error(`${partitionCount} ${topicClientErrors.createTopic_PartitionCountInt}`)
    }
    if (isUndefinedOrNull(replicationFactor)) {
      ...
      console.error(`${replicationFactor} ${topicClientErrors.createTopic_ReplicationFactorInt}`)
    }


    const returnValue = this.zbc.CreateTopic(this.structs.newGoString(topicName), partitionCount, replicationFactor)
    if (returnValue == null) {
      console.error(topicClientErrors.createTopic_NoZbcResponse)
      return null
    } else {
